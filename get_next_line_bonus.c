...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/06/20 21:10:39 by kmonjard          #+#    #+#             */
/*   Updated: 2025/06/20 21:10:40 by kmonjard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	{
		ft_free_buffer(&line_buffer[fd]);
		return (NULL);
	}
	if (!line_buffer[fd])
		line_buffer[fd] = (char *)ft_calloc(1, sizeof(char));
	if (!line_buffer[fd])
		return (NULL);
	update_bool = ft_update_buffer(fd, &line_buffer[fd]);
	if (!update_bool)
		return (NULL);
	new_line = ft_extract_and_retain(&line_buffer[fd]);
	return (new_line);
}
